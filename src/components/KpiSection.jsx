    ...
    <div className="grid gap-6 grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 mb-4">
      {KPI_CARDS_CONFIG.map((config) => {
        return (
          ...
          >
            <p className="text-[12px] font-black capitalize tracking-wider text-slate-500 truncate" title={config.label}>
              {config.label}
            </p>
            <h3 className={`text-4xl font-black tracking-tight mt-2 ${config.textColor}`}>
              {stats[config.key]}
            </h3>
          </div>
        );
