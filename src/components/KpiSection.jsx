  ...
  return (
    <div className="grid gap-6 grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 mb-8">
      {KPI_CARDS_CONFIG.map((config) => (
        <div key={config.label} className={`group flex flex-col justify-between ${config.bgColor} rounded-[20px] p-[20px] shadow-md border border-slate-100/50 transition-all duration-300 hover:-translate-y-1 hover:shadow-xl relative overflow-hidden`}>
