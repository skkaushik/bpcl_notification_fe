...
const Sidebar = () => {
  ...
  return (
    <aside className="sticky top-0 hidden h-screen w-72 border-r border-slate-200 bg-white lg:flex flex-col">
      <div className="flex items-center gap-6 px-9 py-8 border-b border-slate-200">
      ...
      <nav className="flex-2 space-y-9 px-5">
        {navItems.map((item) => (
          <a
            key={item.name}
            href="#"
            className={`flex items-center gap-4 rounded-xl px-5 py-5 text-sm font-semibold transition-all duration-200 ${
              item.active 
